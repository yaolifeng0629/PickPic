...
export function useArticleExtractor() {
  ...
  const { setImages, clearSelection, appendImages } = useImageStore()
  ...
  const extractContent = async () => {
    ...
    try {
      ...
      timerRef.current = setTimeout(async () => {
        try {
          const canvasResponse = await chrome.tabs.sendMessage(targetTabId, { type: 'EXTRACT_CANVAS' })
          if (canvasResponse.canvases?.length > 0) {
            appendImages(canvasResponse.canvases)
          }
        } catch {
          // 忽略补充扫描错误
        }
